...
export class KoiGame {
  ...
  setDirection(next: Direction) {
    ...
    if (next !== OPPOSITE[this.direction]) this.queuedDirection = next;
  }
  ...
  getSnapshot(): KoiSnapshot {
    return {
      ...
      score: this.score,
      best: this.best,
      speedMs: Math.max(108, 148 - Math.floor(this.score / 50) * 8),
  ...
  private randomInt(max: number) {
    ...
    return Math.floor((this.rng / 4294967296) * max);
  }
  ...
  private autoPilot() {
    ...
    if (safe && safe !== OPPOSITE[this.direction]) this.queuedDirection = safe;
    else if (safe) this.queuedDirection = safe;
    void head;
  }

  private distanceAfter(direction: Direction) {
    const delta = DELTA[direction];
    return Math.abs(this.food.x - (this.snake[0].x + delta.x)) + Math.abs(this.food.y - (this.snake[0].y + delta.y));
  }
