...
import { ArrowDown, ArrowLeft, ArrowRight, ArrowUp, Info, Pause, Play, RotateCcw, Sparkles, Volume2, VolumeX, X } from "lucide-react";
import GameCanvas, { GameCanvasHandle } from "../components/GameCanvas";
import { Difficulty, FlowerType, KoiSnapshot } from "../game/koiGame";
...
const defaultSnapshot: KoiSnapshot = { status: "start", snake: [], food: { x: 16, y: 9, type: "white", points: 10 }, score: 0, best: 0, speedMs: 148, lastEaten: false, lastPoints: 0, lastFlower: "white", lastCollision: false, difficulty: "normal" };
const difficultyLabels: Record<Difficulty, { label: string; hint: string }> = { easy: { label: "Brume", hint: "dérive lente · fleurs rares" }, normal: { label: "Jardin", hint: "rythme équilibré" }, hard: { label: "Torrent", hint: "vif · fleurs généreuses" } };
...
export default function Home() {
  ...
  const togglePause = () => { void audioRef.current.unlock(); gameRef.current?.pause(); audioRef.current.playPause(); };
  const playPauseSound = useCallback(() => { void audioRef.current.unlock(); audioRef.current.playPause(); }, []);
  ...
  return (
    <main className="zen-shell">
      <header className="topbar">
        ...
        <div className="top-actions"><span className={`status-pill ${statusTone}`}><span className="status-dot" />{statusLabel}</span><button className="about-button" onClick={() => setAboutOpen(true)}><Info size={14} /> À propos</button><button className="icon-button" aria-label={soundOn ? "Couper le son" : "Activer le son"} onClick={toggleSound}>{soundOn ? <Volume2 size={16} /> : <VolumeX size={16} />}</button><button className="icon-button" aria-label={snapshot.status === "paused" ? "Reprendre" : "Mettre en pause"} onClick={togglePause}>{snapshot.status === "paused" ? <Play size={16} /> : <Pause size={16} />}</button><button className="icon-button" aria-label="Recommencer" onClick={restart}><RotateCcw size={16} /></button></div>
      </header>
      ...
      {aboutOpen && <div className="about-modal" role="dialog" aria-modal="true" aria-labelledby="about-title"><div className="about-card"><button className="about-close" aria-label="Fermer À propos" onClick={() => setAboutOpen(false)}><X size={17} /></button><span className="overlay-japanese">鯉 · 静けさ</span><h3 id="about-title">À propos de Koi Zen Snake</h3><p>Une promenade arcade contemplative imaginée et créée par <strong>Euloge</strong>.</p><div className="about-details"><div><span>CRÉATEUR</span><strong>Euloge</strong></div><div><span>LICENCE</span><strong>MIT · Open Source</strong></div><div><span>TECHNOLOGIES</span><strong>React · TypeScript · Babylon.js</strong></div></div><p className="about-license">Le code peut être utilisé, étudié, modifié et redistribué selon les conditions de la licence MIT.</p><button className="primary-button" onClick={() => setAboutOpen(false)}>Retour au bassin</button></div></div>}
    </main>
  );
}
